...
// 생성자인 constructor안에 모든 코드가 있으면 안된다.
class NewsFeedView extends View{
  ...
  render(): void {
    ...
    for (let i = (store.currentPage - 1) * this.newsLimit; i < store.currentPage * this.newsLimit; i++) {
    ...
    const limit: number = Object.keys(this.feeds).length;
    const divide = Math.floor(limit / this.newsLimit);

    this.setTemplateData('news_feed', this.getHtml());
    this.setTemplateData('prev_page', String(store.currentPage > 1 ? store.currentPage - 1 : 1));
    this.setTemplateData('next_page', String(store.currentPage < divide ? store.currentPage + 1 : store.currentPage));
